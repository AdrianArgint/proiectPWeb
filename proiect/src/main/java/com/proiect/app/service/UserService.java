...
import com.proiect.app.config.Constants;
import com.proiect.app.domain.Authority;
import com.proiect.app.domain.User;
import com.proiect.app.repository.AuthorityRepository;
import com.proiect.app.repository.UserRepository;
import com.proiect.app.security.SecurityUtils;
import com.proiect.app.service.dto.AdminUserDTO;
import com.proiect.app.service.dto.UserDTO;
...
public class UserService {
    ...
    @Transactional(readOnly = true)
    public UserDTO getUserByLogin(String id) {
        return userRepository.findOneByLogin(id).map(user -> new UserDTO(user, placeMapper.toDto(user.getPlace()))).orElse(new UserDTO());
    }
    ...
    @Transactional(readOnly = true)
    public Page<UserDTO> getAllPublicUsersInAPlace(Long placeId, Pageable pageable) {
        return userRepository.findAllByPlace_Id(placeId, pageable).map(UserDTO::new);
    }
    ...
    public UserDTO updateUser(UserDTO userDTO) {
        ...
        if (user.isPresent()) {
            ...
            user.get().setPlace(placeMapper.toEntity(userDTO.getPlace()));
            return new UserDTO(user.get(), userDTO.getPlace());
        }
        return new UserDTO();
    }
