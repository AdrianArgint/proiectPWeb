...
@Component({
  selector: 'jhi-shelter-place',
  templateUrl: './shelter-place.component.html',
})
export class ShelterPlaceComponent implements OnInit {
  places?: IPlace[];
  isLoading = false;
  totalItems = 0;
  ...
  protected onSuccess(data: IPlace[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    ...
    this.page = page;
    if (navigate) {
      this.router.navigate(['/place/shelter'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
