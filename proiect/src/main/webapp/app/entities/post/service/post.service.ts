...
export class PostService {
  ...
  protected convertDateFromClient(post: IPost): IPost {
    return Object.assign({}, post, {
      createdDate: post.createdDate?.isValid() ? post.createdDate.toJSON() : undefined,
      lastUpdatedDate: post.lastUpdatedDate?.isValid() ? post.lastUpdatedDate.toJSON() : undefined,
    });
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((post: IPost) => {
        post.createdDate = post.createdDate ? dayjs(post.createdDate) : undefined;
        post.lastUpdatedDate = post.lastUpdatedDate ? dayjs(post.lastUpdatedDate) : undefined;
      });
    }
    return res;
  }
